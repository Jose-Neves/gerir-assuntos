...
<div class="container">
    ...
    $queryAssuntos = "INSERT INTO `assuntos`
    (`id_assunto`,`assunto_as`,`datahora_as`,`marcadoPor_as`,`aberto_as`)
    VALUES ('','".$AssuntoNovo."','".$DataHoraGrava."','".$MarcadoPor."','1')";
    $dbconn->exec($queryAssuntos);

    $ultimoInserido = $dbconn->lastInsertId('assuntos');
    //$primeiraLinha=$DataHoraGrava.' - Primeira linha de '.$MarcadoPor;
    $queryLinhas = "INSERT INTO `linhas`
    (`id_linha`,`assunto_ln`,`datahora_ln`,`linhatxt_ln`,`insPor_ln`)
    VALUES ('','".$ultimoInserido."','".$DataHoraGrava."','".$AssuntoNovo."','".$MarcadoPor."')";
    $dbconn->exec($queryLinhas);

    }
    catch(PDOException $e)
    {
    echo $queryAssuntos . $queryLinhas . "<br>" . $e->getMessage();
    }
    ?>
